...
const CONSOLE_LOG_TAG_COMP = 'microgateway plugins';
...
function normalizePluginDir(pluginDir) {
  ...
  if (pluginDir) {
    writeConsoleLog('log',{component: CONSOLE_LOG_TAG_COMP},'using pluginDir')
    if (!fs.existsSync(pluginDir)) {
      writeConsoleLog('log',{component: CONSOLE_LOG_TAG_COMP},'pluginDir %s does not exist', pluginDir);
    } else {
      pluginCandidate = path.normalize(pluginDir);
      writeConsoleLog('log',{component: CONSOLE_LOG_TAG_COMP},'using plugin dir %s', pluginCandidate)
    }
  }
...
function _reorderPlugins(dirs, config) {
  ...
  const reordered = sequence.filter(function (plugin) {
    ...
    if (index >= 0) {
      ...
      writeConsoleLog('warn',{component: CONSOLE_LOG_TAG_COMP},'sequenced plugin not found:', plugin);
      return false;
    }
  });
